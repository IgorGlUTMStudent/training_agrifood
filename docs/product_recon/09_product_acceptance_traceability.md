...
| **PA-08** | **Robustness & Data Integrity:** Handling incomplete data | ADR 0002; ADR 0003 (D7) | `status = "insufficient_data"` when the selected engine cannot satisfy its declared minimum assessment inputs; `risk = null`, `deterioration_horizon = null`. Factual `missing_requirements` and `reason_codes` are reported; exact engine minimums and malformed/schema-invalid input handling remain implementation-contract questions. | `ACCEPTED PRODUCT SEMANTICS` / `CURRENT IMPLEMENTED CAPABILITY` (в фикстуре) / `BLOCKED / UNKNOWN` (для операционного SOP) | Статус `status = "insufficient_data"` возникает, когда выбранный движок не может удовлетворить свои заявленные минимальные входные требования для оценки; `risk` и `deterioration_horizon` остаются `null`. Недостающие требования и причины фиксируются фактологически в `missing_requirements` и `reason_codes`. Точные минимальные требования движка и различие между malformed input / schema rejection и incomplete data for assessment остаются вопросом контракта реализации. | 1. Статус `insufficient_data` **СТРОГО НЕ** превращается в `score = 0` или `0.0` и не интерпретируется системой как «безопасная партия».<br>2. `insufficient_data` **СТРОГО НЕ** вызывает автоматическое отклонение партии (auto-reject) или блокировку отгрузки без участия человека.<br>3. **СТРОГО ЗАПРЕЩЕНО** придумывать вымышленный обязательный процесс ручного контроля качества (invented mandatory QC workflow) или автоматически перенаправлять партию на лабораторный анализ. | Тесты бэкенда на возврат `insufficient_data` при невозможности движка удовлетворить минимальные требования оценки; тесты фронтенда на раздельное отображение неоцененных партий; аудит текстов ошибок. | Backend (IGR) / Frontend (Denis) / Integrator Gate (APR2-D3) | «status = insufficient_data when the selected engine cannot satisfy its declared minimum assessment inputs; risk and deterioration_horizon remain null without fabricating scores or forcing auto-rejections.» |
...
| **PA-13** | **Predictive Input Integrity:** Telemetry role & VDR-04B narrative boundary | ADR 0002; ADR 0003 (D10); VDR-04B; APR2-D6 | Telemetry preserved in canonical input (`BatchAssessmentInput`). Product narrative treats C+L baseline as candidate simplification path pending APR2-D6 Human Gate, without overpromising predictive lift from chamber telemetry. | `ACCEPTED INPUT SEMANTICS` / `OBSERVED VDR-04B EVIDENCE` / `PROPOSED NARRATIVE BOUNDARY — REQUIRES HUMAN GATE` | Канонический контракт входных данных (`BatchAssessmentInput`) сохраняет поля телеметрии камер хранения (ADR 0002, ADR 0003 D10). Telemetry remains canonical under ADR 0002 / ADR 0003 D10 and may support contextual monitoring, storage-condition audit, equipment-related diagnostics, or future analytical models. VDR-04B does not validate those uses as implemented Smart Harvest capabilities. Продуктовая документация и коммуникация честно отражают результаты VDR-04B: агрегированная телеметрия в протестированных моделях не дала стабильного маржинального прироста точности поверх плановой логистики; архитектура «культура + логистика» (C+L) предложена как кандидатный путь упрощения pending APR2-D6 Human Gate. | 1. **СТРОГО ЗАПРЕЩЕНО** заявлять в продуктовых материалах, питчах или интерфейсе, что телеметрия камер доказанно повышает точность предсказания потерь (predictive lift) в протестированных моделях.<br>2. **СТРОГО ЗАПРЕЩЕНО** заявлять, что «телеметрия бесполезна» или исключать её из канонической схемы инпута без формального решения об изменении схемы; равно запрещено заявлять, что её полезность для мониторинга/диагностики доказана VDR-04B или реализована в системе.<br>3. **СТРОГО ЗАПРЕЩЕНО** утверждать, что в исследовании VDR-04B окончательно выбраны продакшн-модели или зафиксирован финальный набор признаков.<br>4. **СТРОГО ЗАПРЕЩЕНО** объявлять APR2-D6 принятым до явного решения Integrator (Human Gate). | Проверка схемы Pydantic `BatchAssessmentInput` (наличие полей телеметрии); аудит продуктовой документации, модельных карт и питч-материалов на отсутствие ложных заявлений о предиктивном вкладе телеметрии или преждевременном принятии D6. | Data (Viktor) / Product Owner / Integrator (VLD) / APR2-D6 | «VDR-04B showed that tested aggregate chamber telemetry provided no stable marginal lift over planned logistics; C+L is proposed as a candidate simplification path pending APR2-D6 Human Gate, while telemetry is preserved in canonical input.» |
