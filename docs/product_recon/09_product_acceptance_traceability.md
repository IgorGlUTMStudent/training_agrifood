STATUS: APR2-D1–D6 HUMAN GATE ACCEPTED / TRACEABILITY MATRIX (ADR 0004)
...
> Matrix rows reflecting accepted decisions are updated to accepted MVP scope; independent states (NOT SUPPORTED, DEFERRED, UNKNOWN, IMPLEMENTATION REQUIRED) remain unchanged.
> Implementation-state statements preserved in this matrix describe the historical APR-02F inspected snapshot (refreshed base `cea6ece1d100434772b79b9eb42c8d7bd8488466`) unless a row explicitly states a newer base. They must not be used as authority for current committed implementation state; the application repository is authoritative for current code.
...
**Persona boundary (APR2-D1 / ADR 0004):** FACT: sponsor brief specifies only «farmer or storage operator». DECISION (ADR 0004): primary MVP design persona — **dispatch-side storage operator**; `Dispatch Supervisor` — SoS design label, не sponsor job title. Реальные hold/release полномочия, отмена отгрузки, смена перевозчика/ТС, mandatory QC, подписание CMR и inspection procedure / SOP — UNKNOWN. Advisory Decision Support; compound ramp title не канонизируется.

**Reading this matrix:** DECISION обозначает принятые ADR; OBSERVED RESULT — конкретные offline результаты; INFERENCE — ограниченный вывод; RECOMMENDATION — предложенный scope; UNKNOWN — открытые вопросы. Решения APR2-D1–D6 приняты Human Integrator (Vladimir) 2026-09-21 в [ADR 0004](../decisions/0004-mvp-product-scope.md). В колонке Implementation evidence needed различаются существующие ingestion code/tests и будущие analytics/UI проверки; наличие теста не означает его запуск в APR-02F. IGR-03 canonical ingestion реализован; assessment API по-прежнему возвращает synthetic fixture, без baseline engine или ranked queue.
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **PA-01** | **Outcome 1:** Which batches are most at risk? / Dataset-backed replay prioritisation | ADR 0002; ADR 0003 D1/D3; VDR-01/02; APR2-D2 | Accepted replay membership: `assessment_timestamp = storage_sessions.dispatch_datetime` in a configurable replay/view window. `facility_id` — UI/context filter only. | `ACCEPTED PRODUCT SEMANTICS` (clock/order per ADR 0002/0003) / `ACCEPTED MVP SCOPE` (membership/UI policy per ADR 0004; implementation required) | Assessed batches одной пары `engine_tier + engine_version`: `risk.score DESC, batch_id ASC`. ID tie-break не означает разницу риска. `planned_dispatch_datetime` — plan/schedule context, условно допустимый по ADR 0002, не часы оценки. Duration/capacity UNKNOWN; pagination/scrolling — implementation detail. | Не ранжировать `insufficient_data`, не заменять null нулем, не смешивать движки, не вводить K=10 capacity. Не использовать вымышленные WMS statuses. QC offset в 2 часа — dataset observation, не warehouse operating window. Replay не доказывает live workflow. | Будущие проверки canonical-clock membership, context-only facility filter, same-engine sorting/ties, изоляции unscored batches и UI replay controls. | Backend / Frontend / ADR 0004 (accepted APR2-D2) | «The accepted rule ranks comparable assessed batches; the proposed demo selects recorded dispatch assessments in a configurable replay window. The queue is not implemented.» |
...
1. **Operational Ranking Queue:**
   ...
   - *Gap:* Отсутствует бэкенд-эндпоинт для запроса списка партий с параметрами сортировки и фильтрации по окну/объекту; во фронтенде отсутствует компонент таблицы очереди (Triage Queue). Membership по canonical recorded assessment timestamp и configurable replay/view window приняты как продуктовая политика в ADR 0004 (APR2-D2; implementation required), не live workflow.
   - *Required Future Owner:* Backend (IGR) / Frontend (Denis) / ADR 0004 (accepted APR2-D2).
...
4. **Handling of `insufficient_data` in UI:**
   ...
   - *Gap:* В интерфейсе нет механизма разделения очереди на оценённые партии и партии с неполными данными (вкладка/секция); нет динамического рендеринга списка недостающих требований. Принято раздельное отображение «Not assessed / Incomplete data» (ADR 0004 / APR2-D3; implementation required); операционная обработка UNKNOWN, не определяется APR2-D3.
   - *Required Future Owner:* Frontend (Denis) / ADR 0004 (accepted APR2-D3).
...
5. **Score Method & Separate Batch Context:**
   ...
   - *Required Future Owner:* Backend / Frontend / ADR 0004 (accepted APR2-D4); Outcome 3 остается частично поддержанным.
...
7. **Action Catalogue & Recommendation System:**
   ...
   - *Required Future Owner:* Challenge Sponsor / Domain / ADR 0004 (accepted APR2-D5) — action catalogue *DEFERRED*.
...
| :--- | :--- |
| 1. «ACCEPTED HUMAN DECISION (ADR 0004 / APR2-D1): primary MVP design persona — dispatch-side storage operator; Dispatch Supervisor — SoS design label. Sponsor specifies only farmer or storage operator. Advisory decision support, без установленных реальных полномочий.» | 1. «Система рассчитывает точный час, день или время, когда партия продукции начнет портиться.» *(Биологический онсет не рассчитывается)* |
| 2. «DECISION: `risk.score DESC, batch_id ASC` внутри одной пары `engine_tier + engine_version`; ID tie-break не различает риск. Runtime queue отсутствует; APR2-D2 (ADR 0004) принимает canonical dispatch-event replay window policy.» | 2. «Скор риска 0.82 означает 82%-ю вероятность того, что эта партия сгниет в пути.» *(Скор — монотонная мера потерь, а не вероятность)* |
...
| 7. «FACT: fixture содержит synthetic data_quality factor с effect = unknown. ACCEPTED HUMAN DECISION (ADR 0004 / APR2-D4): baseline factors = [], отдельные How this score is computed и Batch Context; attribution unsupported, Outcome 3 не выполнен полностью.» | 7. «Зеленый статус означает абсолютную безопасность партии, а красный — гарантированный брак.» *(Цветовые диапазоны риска не валидированы и запрещены)* |
...
| 9. «OBSERVED RESULT VDR-04B: tested telemetry aggregates не дали stable marginal lift; некоторые P1 ranking metrics лучше с C+T+L, P2 robustness обычно за C+L. ACCEPTED PRODUCT DECISION (ADR 0004 / APR2-D6): C+L — candidate feature-family simplification для future learned-engine decision; текущее принятое направление — crop-median baseline. Telemetry canonical. Отчет VDR-04B committed, DRAFT FOR REVIEW — EVIDENCE ONLY; не канон.» | 9. «Телеметрия камер хранения бесполезна и может быть удалена из системы» ИЛИ «В исследовании VDR-04B окончательно выбраны продакшн-модели и зафиксирован финальный набор признаков» ИЛИ «Полезность телеметрии для мониторинга/диагностики доказана VDR-04B или реализована в системе.» |
...
3. **Реестр ожидающих UX-зависимостей:**
   - `UX-DEP-1 (Triage Queue UI):` Разработка таблицы ранжирования с поддержкой правила `risk.score DESC, batch_id ASC` (политика принята в ADR 0004 / APR2-D2; реализация в коде требуется).
   - `UX-DEP-2 (Insufficient Data Panel):` Проектирование изолированного представления для партий со статусом `insufficient_data` (политика принята в ADR 0004 / APR2-D3; реализация в коде требуется).
   - `UX-DEP-3 (Card Cleanup):` Очистка `AssessmentCard.tsx` от неактивных полей `risk.band`, `confidence_score` и `starts_at`.
   - `UX-DEP-4 (Score Method, Context & Unavailable Message):` Отдельные «How this score is computed» и «Batch Context», baseline `factors = []`; нейтральный блок `recommendation = null`, не закрывающий Outcome 4 (принято в ADR 0004 / APR2-D4/D5; реализация в коде требуется).
...
- **B. ACCEPTED PRODUCT SEMANTICS:**  
  ADR 0001 (контрактная архитектура); ADR 0002 (граница $T_{dispatch}$, классификация 73 полей, запрет утечек); ADR 0003 (D1: таргет `loss_fraction_pct`, D2: скор тяжести потерь, `band = null`, D3: ранжирование `score DESC, batch_id ASC` внутри одного движка, D5: медианный бейслайн, D6: `horizon = null`, D7: `reliability = unavailable`, D8: допустимы факты с `effect = unknown` либо пустой список, без доказательства attribution, D9: `recommendation = null`, D10: сохранение телеметрии); ADR 0004 (продуктовый скоуп MVP: APR2-D1–D6).
- **C. PROPOSED MVP SCOPE:**  
  Решения APR2-D1–D6 приняты Human Integrator в [ADR 0004](../decisions/0004-mvp-product-scope.md) (dispatch-side storage operator, configurable replay queue policy, baseline `factors = []` с отделением Batch Context, unassessed neutral panel, unavailable recommendation notice, telemetry narrative boundary). Прочие не ратифицированные предложения остаются рекомендациями. C+L зафиксирован строго как candidate simplification для будущего learned-engine decision, не MVP architecture.
