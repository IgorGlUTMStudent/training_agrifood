...
| **Telemetry Narrative Boundary** | ADR 0002 / ADR 0003 (D10): сохранение телеметрии в каноническом инпуте `BatchAssessmentInput`; честная трансляция результатов VDR-04B. | Поля телеметрии присутствуют в схемах; в рантайм-моделях аналитика не сконфигурирована. | Сохранить телеметрию в каноническом инпуте; предложить архитектуру C+L как кандидатный путь упрощения pending APR2-D6 Human Gate; не заявлять о доказанном предиктивном вкладе телеметрии. | Data (Viktor) / Product Owner / Integrator (APR2-D6) | **CURRENT** / **PROPOSED MVP** | Результаты VDR-04B показали отсутствие стабильного маржинального лифта от агрегированной телеметрии. Telemetry remains canonical under ADR 0002 / ADR 0003 D10 and may support contextual monitoring, storage-condition audit, equipment-related diagnostics, or future analytical models, но VDR-04B не валидирует эти применения как внедрённые возможности; телеметрия сохраняется в инпуте, но не переоценивается в MVP. |
...
- **Accepted constraints:**
  - ADR 0002 и ADR 0003 (D10): телеметрия камер хранения строго сохраняется в каноническом инпуте `BatchAssessmentInput`. Telemetry cannot be deleted from canonical input in this task.
  - Запрещено утверждать, что «телеметрия бесполезна». Telemetry remains canonical under ADR 0002 / ADR 0003 D10 and may support contextual monitoring, storage-condition audit, equipment-related diagnostics, or future analytical models. VDR-04B does not validate those uses as implemented Smart Harvest capabilities.
  - Запрещено заявлять в продуктовом нарративе или интерфейсе, что телеметрия доказанно даёт прирост точности (predictive lift) в протестированных моделях.
...
- **Options:**
  ...
  - *Option B (Candidate simplification path / Честный контекст):* В продуктовом нарративе и демонстрациях открыто предложить модель «культура + логистика» (C+L) как кандидатный путь упрощения (simplification path) pending APR2-D6 Human Gate. Позиционировать телеметрию камер как сохранённый канонический инпут под ADR 0002 / ADR 0003 D10, который в будущем может поддерживать контекстный мониторинг или диагностику, честно указав, что в протестированных на данный момент моделях агрегированные сенсорные признаки не показали стабильного маржинального лифта.
