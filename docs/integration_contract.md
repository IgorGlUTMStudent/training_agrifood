...
- **Current reconciliation — VLD-02B:** At `aa5d40bee8368342e7a6b540278c26416cc89457`, VDR-04A is ACCEPTED / INTEGRATED through PR #20. Vladimir explicitly approved ADR 0003 D1–D10 on 2026-09-20, with D3 same-tier/version queue and D4 optional-logistics/no-required-learned-engine overrides. Phase B recording is prepared for Project Brain review, not yet committed/integrated; no implementation is created.
...
This document records the accepted VLD-01 integration contract and workstream dependency map for the SoS team in the Smart Harvest challenge. Its current-state entries include accepted VDR-04A evidence and human-approved ADR 0003 semantics. Phase B recording awaits Project Brain review; architecture and public schema remain unchanged. It connects requirements, domain understanding, dataset reconnaissance, application implementation, evaluation, user experience, demonstration, and final presentation claims. The original pre-recon baseline remains part of project history; the current snapshot and evidence references are recorded in Section 8.
...
Those items were outside the original VLD-01 scope. ADR 0002 subsequently accepted input semantics; ADR 0003 now accepts bounded assessment/evaluation decisions. Remaining choices still require their own evidence and gates.
...
│    Canonical input semantics accepted; IGR-03 awaits IGR-02 integration   │
│                                                                         │
...
- **Downstream work blocked until evidence exists:** Decision gates are task-specific. VLD-02A ([ADR 0002](./decisions/0002-predictive-input-semantics.md)) accepted canonical predictive-input semantics and the 73-field eligibility taxonomy, satisfying the semantic prerequisite for bounded IGR-03 (`BatchAssessmentInput` Pydantic models and CSV mappers). ADR 0003 accepts bounded target/evaluation/baseline and output semantics. Baseline implementation, analytics planning, UX reconciliation and evaluation harness work are conceptually unblocked for subsequent bounded contracts; no production learned engine is selected or required. Cannot authorize backend production models or frontend dashboard implementations ahead of their specific prerequisite evidence.
...
- **Downstream work blocked until evidence exists:** Accepted VDR-01 and VDR-02 satisfied inventory and temporal/leakage prerequisites, and VLD-02A ([ADR 0002](./decisions/0002-predictive-input-semantics.md)) accepted canonical predictive-input semantics, satisfying the semantic prerequisite for IGR-03 canonical models and mapping. ADR 0003 separately accepts target, baseline and evaluation semantics. Exact engine minimums, transformations and imputation/exclusion policy need later contracts; IGR-03 remains subject to IGR-02 integration and cannot contain analytics scoring.
...
| **Assessment-time availability** | B2 (Canonical), B3 (Analytics) | Viktor (VDR-02/04A accepted), Vladimir (ADR 0002/0003) | Audit of all fields at dispatch; list of strictly forbidden future fields (arrival checks, transit delays, final outcomes); shared-zone/chamber leakage audit. | VLD-02A Gate ([ADR 0002](./decisions/0002-predictive-input-semantics.md)): Adopted canonical input semantics, feature eligibility filter, and telemetry boundary ($T_{entry} \le t \le T_{dispatch}$). | Igor: canonical mapping subject to IGR-02 integration; Viktor: harness under accepted P1/P2/P3. | **DECISION:** ADR 0002 eligibility unchanged; ADR 0003 accepts evaluation and optional-logistics capability semantics. IGR-03 depends on IGR-02 integration. Transformations, booking stability and exact fallback remain open. |
...
**Current state — VLD-02B (2026-09-20):** VDR-04A is ACCEPTED / INTEGRATED through PR #20 at `aa5d40bee8368342e7a6b540278c26416cc89457`. Vladimir approved ADR 0003 D1–D10 with D3/D4 overrides. The decision is human-approved; canonical recording awaits Project Brain review and is not yet committed/integrated. Earlier VLD-01/R1/R2/R3 history is preserved.
...
| VLD-02B / ADR 0003 | ACCEPTED HUMAN DECISION; canonical recording pending review | [ADR 0003](./decisions/0003-assessment-evaluation-semantics.md), D1–D10 with D3/D4 overrides | Vladimir explicit Phase B authorization, 2026-09-20; no commit/merge claimed |
...
| PUX-08 | Future data-to-UX reconciliation step; accepted VDR-01–04A, ADR 0003 and APR-01 research are available for inspection | Follow existing PUX workflow; unsupported semantic/UI elements remain gated |
| VLD-02A / IGR-03 predictive input | **INPUT SEMANTICS ACCEPTED; IMPLEMENTATION SEQUENCE STILL APPLIES** | ADR 0002 unchanged. IGR-03 must respect actual IGR-02 integration; ingestion at this base contains only an initializer. No integrated IGR-02 or canonical mapper is claimed, and VLD-02B does not start IGR-03. |
| VLD-02B / ADR 0003 | **HUMAN DECISIONS ACCEPTED; PHASE B RECORDING FOR REVIEW** | Verification then STOP; no implementation/commit/push/PR authorization |
| Bounded deterministic baseline | **CONCEPTUALLY UNBLOCKED** by ADR 0003 | Separate implementation contract; actual input integration, engine minimums and artifact/version handling |
