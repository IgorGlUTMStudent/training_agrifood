...
- **Current reconciliation — VLD-R4:** Reconciled against repository base `98e0df1db0b2987301b6f7523fd45520d67dffa0`. This base incorporates PUX-08 (PR #22, integrated as `DRAFT FOR REVIEW / NOT CANON`), IGR-02 raw ingestion/diagnostics (PR #21), IGR-03 canonical `BatchAssessmentInput` mapping (PR #28), and APR-02F product reconciliation (PR #29, integrated as `PROPOSED PRODUCT SPEC — NOT CANON`, awaiting Human Gate). This update reconciles documentation and implementation metadata with committed reality without changing runtime behaviour or promoting unapproved product proposals to canon.
...
This document records the accepted VLD-01 integration contract and workstream dependency map for the SoS team in the Smart Harvest challenge. Its current-state entries include accepted VDR-04A evidence, human-approved ADR 0003 semantics, integrated PUX-08 UX reconciliation (DRAFT FOR REVIEW), integrated IGR-02 raw ingestion and IGR-03 canonical `BatchAssessmentInput` mapping, and integrated APR-02F (PROPOSED / NOT CANON). Architecture and public schema remain unchanged. It connects requirements, domain understanding, dataset reconnaissance, application implementation, evaluation, user experience, demonstration, and final presentation claims. The original pre-recon baseline remains part of project history; the current snapshot and evidence references are recorded in Section 8.
...
| **Production Ingestion & BatchInput** | `ACCEPTED DECISION (ADR 0002) / IMPLEMENTED IN CODE (IGR-03)` | [`backend/app/domain/batch.py`](../backend/app/domain/batch.py), [`backend/app/ingestion/canonical_mapper.py`](../backend/app/ingestion/canonical_mapper.py), [`docs/decisions/0002-predictive-input-semantics.md`](./decisions/0002-predictive-input-semantics.md) | Canonical `BatchAssessmentInput` semantics, 73-field eligibility matrix, and temporal leakage boundary accepted under ADR 0002; typed domain models and deterministic raw→canonical mapper implemented and tested (IGR-03, PR #28). Enforces $T_{assess} \equiv T_{dispatch}$, restricts telemetry to $T_{entry} \le t \le T_{dispatch}$, preserves missingness as `None`, maps planned logistics, and validates fail-closed joins. Analytics scoring and runtime assessment remain absent. | Vladimir (Decision) / Igor (Implementation) | Ingestion pipeline, Analytics engine (future) | Ingestion produces canonical models for analytics consumption. Production analytics scoring, crop-median runtime engine, and multi-batch endpoints remain future work. | Implementing feature engineering, imputation, or analytics scoring inside the ingestion task. |
| **Analytics Baseline & Deterioration Horizon** | `ACCEPTED SEMANTICS (ADR 0003) / NOT IMPLEMENTED` | [`docs/domain_rules.md`](./domain_rules.md), [`docs/evaluation.md`](./evaluation.md) | **DECISION:** ADR 0003 accepts crop-median baseline, severity score, ranking and evaluation semantics; horizon null. **FACT:** no analytics implementation; fixture still returns `insufficient_data`. | Viktor (D&E) / Alisa (Domain) / Igor (Engine) | Risk Assessment service, Operator UI | Bounded baseline/engine planning under ADR 0003; the median baseline needs no agronomic thresholds. Input integration and engine-specific requirements need later contracts. | Computing synthetic scores or dates without approved baseline definitions and target evidence. |
...
│    ──► ADR 0003 accepted ──► Bounded baseline implementation contract   │
│                                                                         │
...
- **Downstream work blocked until evidence exists:** Implementation of final batch selection tables, risk meters, and recommendation widgets is blocked until Igor delivers assessment endpoints and human product decisions are recorded (APR-02 Human Gate). PUX-08 is integrated as draft evidence, without bypassing implementation or decision prerequisites.
...
**Current state — VLD-R4 (2026-09-21):** At base `98e0df1db0b2987301b6f7523fd45520d67dffa0`, ADR 0003 is ACCEPTED / INTEGRATED, PUX-08 is integrated as `DRAFT FOR REVIEW / NOT CANON` (PR #22), IGR-02 raw ingestion/diagnostics is ACCEPTED / INTEGRATED (PR #21), IGR-03 canonical `BatchAssessmentInput` mapping is ACCEPTED / INTEGRATED (PR #28), and APR-02F product reconciliation is integrated as `PROPOSED PRODUCT SPEC — NOT CANON` (PR #29, awaiting Human Gate). Earlier VLD-01/R1/R2/R3/02B history is preserved.
...
| APR-02F | INTEGRATED AS PROPOSED PRODUCT SPEC / NOT CANON (requires Human Gate) | [08_mvp_scope_decision_packet.md](./product_recon/08_mvp_scope_decision_packet.md), [09_product_acceptance_traceability.md](./product_recon/09_product_acceptance_traceability.md) | [PR #29](https://github.com/Slave-of-Skynet/training_agrifood/pull/29) |

Acceptance of a research/report artifact does not adopt all its recommendations as DECISION or prove implementation. APR-01 research documents retain their own epistemic labels (`FACT`, `DOMAIN FACT`, `INFERENCE`, `UNKNOWN`, `NOT CANON`) and are not automatically challenge canon as a whole. PUX-08 is integrated as draft evidence, not accepted product canon. APR-02F remains proposed scope requiring explicit Human Gate acceptance from Vladimir. IGR-01 records historical backend verification and host-specific runtime/build limitations; PUX distinguishes planned QA from executed checks. No fresh application verification is claimed by this documentation update.
...
| VLD-02A / IGR-03 predictive input | **COMPLETE / ACCEPTED / INTEGRATED** through [PR #28](https://github.com/Slave-of-Skynet/training_agrifood/pull/28) | Typed canonical `BatchAssessmentInput` domain models and deterministic raw-to-canonical mapper are operational in code/tests. Analytics scoring runtime remains absent. |
| VLD-02B / ADR 0003 | **ACCEPTED HUMAN DECISION; INTEGRATED** | Assessment, ranking, baseline, and evaluation semantics accepted |
| APR-02F product spec | **INTEGRATED AS PROPOSED PRODUCT SPEC / NOT CANON** through [PR #29](https://github.com/Slave-of-Skynet/training_agrifood/pull/29) | Requires explicit Human Gate from Integrator (Vladimir) before APR2-D1–D6 become project decisions |
| Bounded deterministic baseline | **CONCEPTUALLY UNBLOCKED** by ADR 0003; canonical inputs available via IGR-03 | Separate implementation contract; actual input integration, engine minimums and artifact/version handling |
...
| Evaluation harness | **CONCEPTUALLY UNBLOCKED** | Implement P1/P2/P3, baseline and metrics under a separate contract; preserve leakage boundaries |
...
The original VLD-01 limitations are reconciled below against accepted evidence and ADR 0003; accepted semantics are distinguished from absent implementation:
...
2. **Canonical Input Models & Mapper Implemented (IGR-03); Analytics Runtime Absent:** Canonical `BatchAssessmentInput` domain models and deterministic raw-to-canonical mapper are implemented in `backend/app/domain/batch.py` and `backend/app/ingestion/canonical_mapper.py` (PR #28). However, production analytics scoring runtime, runtime crop-median engine execution, ranked queue endpoints, recommendations, and learned models are not yet implemented in application code.
3. **Risk Semantics Accepted, Not Implemented:** ADR 0003 accepts crop-median baseline and bounded severity score. The backend still calculates no risk scores; the demo remains a synthetic `insufficient_data` fixture.
