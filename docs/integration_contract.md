...
- **Status:** DRAFT — VLD-01 EVIDENCE READY / PENDING HUMAN INTEGRATION
...
This document defines the draft integration contract and workstream dependency map for the SoS team in the Smart Harvest challenge, pending human review and integration by Vladimir. It connects requirements, domain understanding, dataset reconnaissance, application implementation, evaluation, user experience, demonstration, and final presentation claims.
...
| **Production Ingestion & BatchInput** | `DOCUMENTED BUT NOT YET IMPLEMENTED/ENFORCED` | [`docs/data_contract.md`](./data_contract.md), [`sponsor_pack/data/`](../sponsor_pack/data/) | Raw CSV data exists. Canonical `BatchInput` schema intentionally omitted from foundation; no ingestion parser exists in code. | Viktor (Recon) / Igor (Implementation) | Ingestion pipeline, Analytics engine | Factual raw parsing and structural validation helpers may be implemented in a bounded task once VDR-01 measured schema/integrity evidence is accepted. Predictive canonical models (`BatchAssessmentInput`) and leakage-sensitive transformations strictly require accepted VDR-02 evidence and a VLD-02 decision gate. | Constructing canonical Pydantic input models or establishing predictive mapping prior to VLD-02 approval of input semantics. |
...
│    VDR-01 + VDR-02 Accepted ──► VLD-02 Decision ──► Canonical           │
│    BatchAssessmentInput & predictive mapping                            │
│                                                                         │
...
- **Accepted output that currently exists:** Workstream contract [`docs/workstreams/viktor-data-recon.md`](./workstreams/viktor-data-recon.md); supplied dataset files in `sponsor_pack/data/`.
- **Future planned output:** `docs/data_recon/01_dataset_inventory.md` (VDR-01), `docs/data_recon/02_temporal_leakage.md` (VDR-02), and subsequent evaluation design, baseline results, and model evaluation reports.
...
- **Downstream work blocked until evidence exists:** `VDR-02` starts only after reviewed `VDR-01`. Factual raw parsing helpers require accepted VDR-01. Deciding predictive canonical input semantics (`BatchAssessmentInput`) requires accepted VDR-01 (integrity) and VDR-02 (leakage/availability) evidence. Later evaluation and model work requires separately contracted Viktor evidence.
...
- **Accepted output that currently exists:** FastAPI application skeleton (`backend/app/main.py`, `backend/app/api/routes.py`), Pydantic output models (`backend/app/domain/assessment.py`), synthetic demo fixture (`backend/app/services/demo_assessment.py`), backend tests (`backend/tests/`), workstream contract [`docs/workstreams/igor-technical-recon.md`](./workstreams/igor-technical-recon.md).
- **Future planned output:** Technical reconnaissance report (`docs/recon/IGR-01-technical-recon.md`), ingestion implementation (`backend/app/ingestion/`), analytics engine (`backend/app/analytics/`), explainability service (`backend/app/explain/`), recommendation service (`backend/app/recommend/`), batch assessment API routes.
- **Who consumes output:** prscr (API responses for UI consumption), Vladimir (system coherence and test verification).
- **Downstream work blocked until evidence exists:** Factual raw parsing helpers may proceed once VDR-01 integrity evidence is accepted in a bounded implementation task. Cannot implement predictive canonical models (`BatchAssessmentInput`) or leakage-sensitive mappings until VDR-01 and VDR-02 evidence is approved and canonical input semantics are accepted at a VLD-02 gate. Cannot build analytics engines or risk logic until VLD-02 approves target/evaluation semantics and method-specific baseline rules.
...
- **Accepted output that currently exists:** React+TS+Vite shell (`frontend/src/`), connection state management (`HomePage.tsx`), synthetic assessment rendering (`AssessmentCard.tsx`), health status display (`BackendStatus.tsx`), typed API client (`api/client.ts`, `api/contracts.ts`), workstream contract [`docs/workstreams/prscr-ux-demo-qa.md`](./workstreams/prscr-ux-demo-qa.md).
- **Future planned output:** UX reconnaissance & research notes (`docs/recon/PUX-00-07-ux-demo-qa.md`), wireframe specifications, interactive batch dashboard, detailed factor inspection UI, demo flow execution, and automated/exploratory QA test results.
...
- **Accepted output that currently exists:** Workstream contract [`docs/workstreams/alisa-product-recon.md`](./workstreams/alisa-product-recon.md).
- **Future planned output:** Product and requirements reconnaissance report, operator decision model, product workflow specification, domain research notes with verified citations, candidate rule reviews for `docs/domain_rules.md`, and pitch deck narrative structure.
...
- **Downstream work blocked until evidence exists:** Final recommendation labels and operator action scopes are blocked until external agricultural and post-harvest sources are reviewed. Product/domain evidence is required only for decisions that depend on product/domain meaning (including agronomic thresholds if required by the chosen baseline). Pitch claims regarding economic loss reduction are blocked until evaluation evidence is obtained.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **B1: Raw Dataset → Ingestion** | Sponsor pack (`sponsor_pack/data/*.csv`) | Ingestion layer (`backend/app/ingestion/`) | Documented CSV schemas and data dictionary (`sponsor_pack/README.md`, `data_dictionary.xlsx`). | **UNVERIFIED RAW** (Files supplied, empirical integrity unmeasured). | Field names and types, Joins and entity relationships, Dataset size | Viktor (`VDR-01`) | Measured row/column counts, parse exceptions, null rates, candidate PK uniqueness, orphan counts, join cardinalities across all 8 tables. | Vladimir (Integration Gate / VLD-02 if shared contract changes) | Writing CSV loaders or ingestion pipelines based on documentation assumptions; silent data loss or crash on unexpected values. Accepted VDR-01 evidence is sufficient to unblock a bounded implementation task for factual raw parsing and validation helpers reflecting measured schema, without establishing predictive canonical semantics. |
| **B2: Ingestion → Canonical Representation** | Ingestion layer (`backend/app/ingestion/`) | Canonical entity models (future `app/domain/batch.py` or similar) | **NONE ACCEPTED.** Foundation intentionally omits production `BatchInput`. | **PENDING RECON & DECISION** | Field names and types, Joins and entity relationships, Assessment-time availability, Missing and noisy data | Viktor (`VDR-01`, `VDR-02`) | Verified entity grain (facility, zone, batch, session), resolved join paths (session to sensor), explicit dispatch-time availability and leakage audit. VDR-02 is mandatory before predictive canonical input semantics are finalized. | Vladimir (VLD-02 Decision Gate to accept semantics; Igor implements schema in subsequent task) | Constructing canonical Pydantic input models (`BatchAssessmentInput`); guessing field names; binding models to unverified raw columns; leaking post-dispatch fields into canonical predictive inputs. |
| **B3: Canonical Representation → Analytics** | Canonical data layer | Analytics engine (`backend/app/analytics/`) | **NONE ACCEPTED.** Foundation requires deterministic baseline first; ML optional. | **BLOCKED ON EVIDENCE** | Measurement frequency and time series, Assessment-time availability, Outcomes and candidate labels, Deterioration timing, Agronomic rules and thresholds | Viktor (`VDR-02`, later D&E) & Alisa (domain, conditionally) | Leakage audit confirming forbidden post-dispatch fields; defined and defensible target; temporal window rules; baseline logic definition based on method-specific evidence (with domain rules required conditionally if baseline relies on agronomic thresholds). | Vladimir (VLD-02 Decision Gate to accept baseline logic; Igor implements in subsequent task) | Training ML models on future/leaked data; predicting undefined targets; hardcoding unvalidated risk heuristics or unsourced agronomic thresholds. |
...
| --- | --- | --- | --- | --- | --- | --- |
| **Field names and types** | B1 (Ingestion), B2 (Canonical) | Viktor (`VDR-01`) | Measured row counts, parsing exceptions, column null percentages, type conformance across all 8 tables. | VLD-02 Gate: Adopt raw ingestion parsing and type coercion rules if shared contracts are affected; accepted VDR-01 unblocks bounded raw parsing helpers. | Igor: Factual raw parsing helpers and validator implementation (unblocked by accepted VDR-01). | **PENDING VDR-01** |
| **Joins and entity relationships** | B1 (Ingestion), B2 (Canonical) | Viktor (`VDR-01`) | Tested join integrity among facilities, zones, sessions, batches, checks, shipments, telemetry, outcomes. Orphan counts. | VLD-02 Gate: Adopt canonical entity relationships and join strategies. | Igor: Canonical data structure and relational join implementation. | **PENDING VDR-01** |
| **Dataset size** | B1 (Ingestion), Storage architecture | Viktor (`VDR-01`), Igor (`IGR-01`) | Exact file sizes, memory footprint when loading tables (especially `sensor_readings.csv`), query latency. | VLD-02 Gate: Decide whether in-memory processing suffices or indexed storage is required. | Igor: Data loading architecture; Vladimir: Operational demo hardware requirements. | **PENDING VDR-01 / IGR-01** |
| **Measurement frequency and time series** | B2 (Canonical), B3 (Analytics) | Viktor (`VDR-01`, `VDR-02`) | Actual sampling intervals (nominal 30 min), timestamp continuity, gap frequencies, out-of-session readings. | VLD-02 Gate: Adopt sensor aggregation windows and missing-telemetry imputation/rejection rules. | Igor: Time-series feature extraction pipelines; Viktor: Baseline feature definitions. | **PENDING VDR-01 / VDR-02** |
| **Assessment-time availability** | B2 (Canonical), B3 (Analytics) | Viktor (`VDR-02`) | Audit of all fields at dispatch; list of strictly forbidden future fields (arrival checks, transit delays, final outcomes); shared-zone/chamber leakage audit. | VLD-02 Gate: Accept canonical input semantics and feature eligibility filter respecting fixed constraint $T_{assess} = T_{dispatch}$. | Igor: Bounded implementation task for predictive `BatchAssessmentInput` and leakage-safe transformations (strictly blocked on VDR-02 and VLD-02); Viktor: Leakage-free split design. | **PENDING VDR-02** |
| **Outcomes and candidate labels** | B3 (Analytics), B4 (RiskAssessment) | Viktor (later D&E contract) & Alisa (`APR-01`) | Distribution of outcome fields (`quality_status`, `loss_fraction_pct`, `quality_score`, `economic_loss_eur`); definition of business risk target. | VLD-02 Gate: Adopt formal analytical target definition and evaluation metrics. | Viktor: Model training & evaluation; Igor: Analytics engine risk calculation; prscr: UI risk gauge. | **PENDING D&E CONTRACT** |
| **Deterioration timing** | B3 (Analytics), B4 (RiskAssessment) | Viktor (later D&E contract) & Alisa (`APR-01`) | Analysis of whether intermediate `quality_checks` or telemetry provide supportable onset timestamps or shelf-life proxies. | VLD-02 Gate: Decide whether `deterioration_horizon` can be computed or must remain null. | Igor: Horizon estimation logic; prscr: Timeline visualization on frontend. | **PENDING D&E CONTRACT** |
| **Intervention and action effects** | B3 (Analytics), B4 (RiskAssessment) | Alisa (`APR-01`), Viktor (later D&E contract) | Sourced domain literature on post-harvest interventions; check if dataset contains action-outcome pairs. | VLD-02 Gate: Adopt structured `action_code` catalog and human review guardrails. | Igor: Recommendation service; prscr: Action recommendation cards. | **PENDING APR-01 & D&E** |
| **Monetary and quantity fields** | B3 (Analytics), B8 (Pitch) | Viktor (`VDR-01`), Alisa (`APR-01`) | Verification of `economic_loss_eur` derivation and validity for business impact claims. | VLD-02 Gate: Decide if financial impact estimation is supportable in MVP. | Alisa: Pitch ROI claims; Igor: Potential financial loss display. | **PENDING VDR-01 & APR-01** |
| **Crop diversity** | B2 (Canonical), B3 (Analytics) | Viktor (`VDR-01`), Alisa (`APR-01`) | Distribution of crop types in batches; domain storage requirements per crop. | VLD-02 Gate: Decide single-crop MVP focus or multi-crop segmentation. | Igor: Model/rule scoping; Viktor: Subgroup evaluation protocol. | **PENDING VDR-01 & APR-01** |
| **Missing and noisy data** | B2 (Canonical), B4 (RiskAssessment) | Viktor (`VDR-01`), Igor (`IGR-01`) | Frequency of missing sensor readings or missing checks; criteria for triggering `insufficient_data`. | VLD-02 Gate: Adopt minimum data sufficiency requirements for an assessment. | Igor: Invariant enforcement in application services; prscr: Degraded UI rendering. | **PENDING VDR-01 / IGR-01** |
| **Realtime source** | B1 (Ingestion), B5 (API) | Alisa (`APR-01`), Igor (`IGR-01`) | Confirmation of operator workflow needs (batch upload vs scheduled batch vs simulated stream). | VLD-02 Gate: Confirm MVP runtime processing mode (retains batch/on-demand by default). | Igor: API route design; prscr: Upload/run workflow in UI. | **PENDING APR-01 / IGR-01** |
| **Hosting constraints** | B7 (Demo) / Runtime Environment | Vladimir (`VLD-04`), Igor (`IGR-01`), prscr (Demo needs) | Target demonstration environment needs, network connectivity requirements, hardware resources, deployment complexity. | VLD-04 Gate: Environment and platform selection remains under VLD-04 unless an earlier blocker requires a human decision. No hosting provider preselected; remote hosting not required by default. | Vladimir: Demo deployment runbook, final runtime environment setup. | **PENDING VLD-04 / DEMO RECON** |
| **Agronomic rules and thresholds** | B3 (Analytics), `docs/domain_rules.md` | Alisa (`APR-01`), Viktor (`VDR-01`) | Primary literature sources (FAO, university extension) for temperature/humidity thresholds; data compatibility. | VLD-02 Gate: Formal admission of candidate rules into `docs/domain_rules.md`. | Igor: Deterministic baseline rule implementation (if baseline design relies on agronomic thresholds); prscr: Explanatory factor text. | **PENDING APR-01** |
| **Learned model family** | B3 (Analytics) | Viktor (later D&E contract) | Measured performance of deterministic baseline; empirical proof that ML provides meaningful improvement. | VLD-02 Gate: Authorize learned tier implementation in `backend/app/analytics/`. | Igor: Model serialization and inference pipeline integration. | **PENDING D&E CONTRACT** |
| **Persistence needs** | Architecture, Storage layer | Igor (`IGR-01`), Viktor (`VDR-01`), Alisa (`APR-01`) | Volume, lifecycle, concurrency, audit, and operator workflow needs; whether in-memory processing suffices or disk/DB persistence is needed. | VLD-02 Gate: May become a VLD-02 cross-cutting decision if accepted evidence shows persistence is required for application implementation. | Igor: Database setup/storage layer (if needed); Vladimir: Architecture consistency. | **PENDING RECON EVIDENCE** |
...
As of this baseline (Starting SHA: `cafbcf832e7abb410ee9ff07954b4bb50e34aa81`), the operational status of all major challenge tasks is mapped below:

```text
┌────────────────────────────────────────────────────────────────────────┐
│                        CURRENTLY UNBLOCKED                             │
│                                                                        │
│  [ACTIVE] VLD-01: Integration Contract (Vladimir - this task)          │
│  [READY]  VDR-01: Dataset Inventory & Integrity Recon (Viktor)         │
│  [READY]  IGR-01: Technical Reconnaissance & Backend Map (Igor)        │
│  [READY]  APR-01: Product & Requirements Recon (Alisa)                 │
│  [READY]  PUX-00-07: UX Recon, Flow & QA Planning Phase (prscr)        │
└──────────────────────────────────┬─────────────────────────────────────┘
                                   │
                                   ▼ Produces Recon Evidence
┌────────────────────────────────────────────────────────────────────────┐
│                   BLOCKED PENDING INITIAL RECON                        │
│                                                                        │
│  [BLOCKED] IGR-02: Factual Raw Parsing & Schema Validation Helpers     │
│            └─ Blocked on: Accepted VDR-01 inventory report             │
│  [BLOCKED] VDR-02: Temporal Semantics & Leakage Audit                  │
│            └─ Blocked on: Accepted VDR-01 inventory report             │
│  [BLOCKED] VLD-02A: Canonical Predictive Input Semantics Decision Gate │
│            └─ Blocked on: Accepted VDR-01 & VDR-02 evidence packets    │
│  [BLOCKED] IGR-03: Canonical BatchAssessmentInput Implementation       │
│            └─ Blocked on: VLD-02A decision approval                    │
└──────────────────────────────────┬─────────────────────────────────────┘
                                   │
                                   ▼ Produces Baseline & Leakage Constraints
┌────────────────────────────────────────────────────────────────────────┐
│                   BLOCKED PENDING EVALUATION EVIDENCE                  │
│                                                                        │
│  [BLOCKED] Later Viktor Contracts: Target, Split, Metrics Proposals    │
│            └─ Blocked on: VDR-02 leakage audit & target analysis       │
│  [BLOCKED] VLD-02B: Analytics Baseline Decision Gate                   │
│            └─ Blocked on: Target/split approval & method evidence      │
│               (domain rules conditional on agronomic threshold use)   │
│  [BLOCKED] Backend Analytics Engine Implementation (IGR)               │
│            └─ Blocked on: VLD-02B decision approval                    │
│  [BLOCKED] Frontend Interactive Dashboard Implementation (prscr)       │
│            └─ Blocked on: Backend assessment endpoints & wireframes    │
│  [BLOCKED] VLD-03: End-to-End Integration Gate                         │
│            └─ Blocked on: Working backend + frontend integration       │
│  [BLOCKED] VLD-04: Runtime Readiness & Demo Operationalization         │
│            └─ Blocked on: Integrated product stability                 │
│  [BLOCKED] VLD-05: Final Consistency & Pitch Claim Gate                │
│            └─ Blocked on: Complete solution, evaluation & demo script  │
└────────────────────────────────────────────────────────────────────────┘
```

> [!NOTE]
> The `PUX-00–07` research/design reconnaissance phase may proceed under its own sequential workflow. This does not imply that subsequent frontend implementation steps can bypass upstream backend assessment contracts or internal UX prerequisites.
...
1. **No Ingestion Logic Created:** The raw CSV files under `sponsor_pack/data/` remain unparsed by application code. Their empirical structure will be established by Viktor in VDR-01.
2. **No Canonical `BatchInput` Defined:** Production/canonical input semantics have not been accepted, and no Pydantic input models exist in code. Canonical input semantics may be evaluated and decided at a future VLD-02 decision gate once required Viktor evidence (VDR-01 and VDR-02) is accepted; actual Pydantic schema and application implementation must occur only in a subsequent bounded implementation task following that decision.
