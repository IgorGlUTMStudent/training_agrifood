...
- **Reconciliation:** VLD-R1 working update against accepted, integrated Layer-1 evidence at `54d233f1822f730a26fa225ec00d746e116875aa`, prepared for human review on 2026-09-19. This documentation reconciliation makes no new VLD-02 decision.
...
- **Accepted output that currently exists:** [`docs/data_recon/01_dataset_inventory.md`](./data_recon/01_dataset_inventory.md) (VDR-01, PR #7); workstream contract [`docs/workstreams/viktor-data-recon.md`](./workstreams/viktor-data-recon.md). The supplied dataset remains the source snapshot.
- **Pending work / future output:** VDR-02 (`docs/data_recon/02_temporal_leakage.md`) is **FIX REQUIRED / NOT ACCEPTED / NOT INTEGRATED / NOT CANON**; one final targeted microfix remains. Subsequent evaluation design, baseline results, and model evaluation reports are future work. No pending VDR-02 findings are adopted here.
...
- **Downstream work blocked until evidence exists:** VDR-01 has satisfied the inventory prerequisite for VDR-02 and for a separately contracted factual raw parsing task. VDR-02 itself is not accepted. Deciding predictive canonical input semantics (`BatchAssessmentInput`) still requires accepted VDR-02 and a VLD-02 decision. Later evaluation and model work requires separately contracted Viktor evidence.
...
- **Downstream work blocked until evidence exists:** Accepted VDR-01 now satisfies the evidence prerequisite for a separately contracted raw-only IGR-02. This does not authorize production `BatchAssessmentInput`, predictive eligibility decisions, feature windows, imputation/exclusion policy, target selection, or analytics logic. Predictive canonical models and leakage-sensitive mappings remain blocked on accepted VDR-02 and VLD-02 input approval; analytics remains blocked on accepted target/evaluation semantics and method-specific baseline evidence.
...
- **Current APR-01 status:** Repository state **NOT DELIVERED / NOT INTEGRATED**; project work state **PARTIAL / IN PROGRESS**; acceptance **NOT ACCEPTED**; canonical status **NOT CANON**. This records the project-owner workflow clarification, not acceptance of external research content.
...
| **B2: Ingestion → Canonical Representation** | Ingestion layer (`backend/app/ingestion/`) | Canonical entity models (future `app/domain/batch.py` or similar) | **BatchAssessmentInput: NOT ACCEPTED.** Foundation intentionally omits production input semantics. | **PREDICTIVE MAPPING / DETAILED ELIGIBILITY BLOCKED** on accepted VDR-02 and VLD-02. | Field names and types, Joins and entity relationships, Assessment-time availability, Missing and noisy data | Viktor (`VDR-01` accepted; `VDR-02` NOT ACCEPTED) | VDR-01 supplies observed grain and join paths; accepted dispatch-time availability/leakage evidence and an input decision are still required. | Vladimir (VLD-02 Decision Gate to accept semantics; Igor implements schema in subsequent task) | Constructing canonical Pydantic input models or predictive mappings before approval; treating measured raw relationships as accepted predictive semantics. |
| **B3: Canonical Representation → Analytics** | Canonical data layer | Analytics engine (`backend/app/analytics/`) | **NONE ACCEPTED.** Foundation requires deterministic baseline first; ML optional. | **BLOCKED ON EVIDENCE:** analytical target UNKNOWN; baseline and evaluation protocol NOT ACCEPTED. | Measurement frequency and time series, Assessment-time availability, Outcomes and candidate labels, Deterioration timing, Agronomic rules and thresholds | Viktor (`VDR-02`, later D&E) & Alisa (domain, conditionally) | Accepted VDR-02; separately approved target/evaluation evidence; temporal window rules and method-specific baseline evidence (domain rules required conditionally if the baseline relies on agronomic thresholds). | Vladimir (VLD-02 Decision Gate to accept baseline logic; Igor implements in subsequent task) | Training ML models on future/leaked data; predicting undefined targets; hardcoding unvalidated risk heuristics or unsourced agronomic thresholds. |
...
| --- | --- | --- | --- | --- | --- | --- |
| **Field names and types** | B1 (Ingestion), B2 (Canonical) | Viktor (`VDR-01`) | Measured row counts, parsing exceptions, column null percentages, type conformance across all 8 tables. | VLD-02 Gate: Adopt raw ingestion parsing and type coercion rules if shared contracts are affected; accepted VDR-01 unblocks bounded raw parsing helpers. | Igor: Factual raw parsing helpers and validator implementation (unblocked by accepted VDR-01). | **VDR-01 ACCEPTED:** observed schema/types/nullability measured; bounded raw-only task may be separately contracted. Production validation/coercion and unseen-value policy remain UNKNOWN. |
| **Joins and entity relationships** | B1 (Ingestion), B2 (Canonical) | Viktor (`VDR-01`) | Tested join integrity among facilities, zones, sessions, batches, checks, shipments, telemetry, outcomes. Orphan counts. | VLD-02 Gate: Adopt canonical entity relationships and join strategies. | Igor: Canonical data structure and relational join implementation. | **VDR-01 ACCEPTED:** snapshot integrity/cardinalities measured. Canonical predictive mapping and unexpected-cardinality handling remain blocked on accepted VDR-02 / VLD-02. |
...
| **Measurement frequency and time series** | B2 (Canonical), B3 (Analytics) | Viktor (`VDR-01`, `VDR-02`) | Actual sampling intervals (nominal 30 min), timestamp continuity, gap frequencies, out-of-session readings. | VLD-02 Gate: Adopt sensor aggregation windows and missing-telemetry imputation/rejection rules. | Igor: Time-series feature extraction pipelines; Viktor: Baseline feature definitions. | **VDR-01 ACCEPTED:** sampling/gaps/coverage measured. Feature windows, staleness tolerance and treatment await accepted VDR-02 and later decisions. |
| **Assessment-time availability** | B2 (Canonical), B3 (Analytics) | Viktor (`VDR-02`) | Audit of all fields at dispatch; list of strictly forbidden future fields (arrival checks, transit delays, final outcomes); shared-zone/chamber leakage audit. | VLD-02 Gate: Accept canonical input semantics and feature eligibility filter respecting fixed constraint $T_{assess} = T_{dispatch}$. | Igor: Bounded implementation task for predictive `BatchAssessmentInput` and leakage-safe transformations (strictly blocked on VDR-02 and VLD-02); Viktor: Leakage-free split design. | **VDR-02 FIX REQUIRED / NOT ACCEPTED:** detailed eligibility and predictive mapping remain blocked; sponsor dispatch constraint is unchanged. |
| **Outcomes and candidate labels** | B3 (Analytics), B4 (RiskAssessment) | Viktor (later D&E contract) & Alisa (`APR-01`) | Distribution of outcome fields (`quality_status`, `loss_fraction_pct`, `quality_score`, `economic_loss_eur`); definition of business risk target. | VLD-02 Gate: Adopt formal analytical target definition and evaluation metrics. | Viktor: Model training & evaluation; Igor: Analytics engine risk calculation; prscr: UI risk gauge. | **VDR-01 ACCEPTED:** outcome presence/distributions/selected historical relationships known. Primary target, score meaning, label fitness and evaluation remain UNKNOWN / PENDING LATER D&E. |
| **Deterioration timing** | B3 (Analytics), B4 (RiskAssessment) | Viktor (later D&E contract) & Alisa (`APR-01`) | Analysis of whether intermediate `quality_checks` or telemetry provide supportable onset timestamps or shelf-life proxies. | VLD-02 Gate: Decide whether `deterioration_horizon` can be computed or must remain null. | Igor: Horizon estimation logic; prscr: Timeline visualization on frontend. | **PENDING LATER D&E / RELEVANT PRODUCT EVIDENCE:** no accepted onset or horizon semantics. |
| **Intervention and action effects** | B3 (Analytics), B4 (RiskAssessment) | Alisa (`APR-01`), Viktor (later D&E contract) | Sourced domain literature on post-harvest interventions; check if dataset contains action-outcome pairs. | VLD-02 Gate: Adopt structured `action_code` catalog and human review guardrails. | Igor: Recommendation service; prscr: Action recommendation cards. | **APR-01 PARTIAL / NOT ACCEPTED; LATER D&E PENDING:** no accepted action/effect semantics. |
| **Monetary and quantity fields** | B3 (Analytics), B8 (Pitch) | Viktor (`VDR-01`), Alisa (`APR-01`) | Verification of `economic_loss_eur` derivation and validity for business impact claims. | VLD-02 Gate: Decide if financial impact estimation is supportable in MVP. | Alisa: Pitch ROI claims; Igor: Potential financial loss display. | **VDR-01 ACCEPTED:** historical formula reproduced within reported tolerance. Business interpretation, prevented loss, ROI and causal savings remain UNKNOWN; APR-01 not accepted. |
| **Crop diversity** | B2 (Canonical), B3 (Analytics) | Viktor (`VDR-01`), Alisa (`APR-01`) | Distribution of crop types in batches; domain storage requirements per crop. | VLD-02 Gate: Decide single-crop MVP focus or multi-crop segmentation. | Igor: Model/rule scoping; Viktor: Subgroup evaluation protocol. | **VDR-01 ACCEPTED:** crop/cultivar coverage and counts measured. Transferability, rule scope and subgroup sufficiency remain UNKNOWN; APR-01 not accepted. |
...
| Shared chamber / zone-time dependence | B2, B3 | Accepted VDR-01 establishes concurrent chamber sharing | Accepted VDR-02 plus later split/evaluation evidence and decision, including residual facility/zone dependence; no claim that cluster blocking solves leakage | Viktor → Vladimir; analytics/evaluation consumers |
...
**History:** VLD-01 was prepared from `cafbcf832e7abb410ee9ff07954b4bb50e34aa81`, before the subsequent evidence deliveries. The earlier pending-state map described that baseline. The following current-state reconciliation uses `54d233f1822f730a26fa225ec00d746e116875aa` and GitHub state checked on 2026-09-19; it does not rewrite the earlier sequence or introduce a VLD-02 decision.

### Accepted and integrated Layer-1 evidence
...
Acceptance of a research/report artifact does not adopt all its recommendations as DECISION or prove implementation. IGR-01 records historical backend verification and host-specific runtime/build limitations; PUX distinguishes planned QA from executed checks. No fresh application verification is claimed by this documentation update.

### Pending external work — workflow status only

The project-owner clarification for VLD-R1, recorded 2026-09-19, supplements repository visibility. Only workflow status is recorded; no external findings or recommendations are admitted as accepted evidence.

| Workstream | Repository state | Project work state | Acceptance / canonical status | Remaining gate |
| --- | --- | --- | --- | --- |
| VDR-02 | NOT INTEGRATED | FIX REQUIRED; one final targeted microfix outstanding | NOT ACCEPTED / NOT CANON | Fix → human review and acceptance → human integration; [Issue #10](https://github.com/Slave-of-Skynet/training_agrifood/issues/10) is open / FIX REQUIRED |
| APR-01 | NOT DELIVERED / NOT INTEGRATED | PARTIAL / IN PROGRESS | NOT ACCEPTED / NOT CANON | Continue scoped research → review/acceptance of relevant evidence → human integration; [Issue #1](https://github.com/Slave-of-Skynet/training_agrifood/issues/1) is open / IN PROGRESS |

### Readiness under existing task-specific gates

| Work | Current readiness | Required next evidence / gate |
| --- | --- | --- |
| Bounded raw-only IGR-02 | **VDR-01 prerequisite satisfied; may be issued as a separate bounded contract.** No implementation exists or is created by VLD-R1 | Factual raw parsing / structural validation only, under Section 3's existing rule |
| VDR-02 | **FIX REQUIRED**, not waiting for VDR-01 | Final targeted microfix and human acceptance; pending content cannot satisfy another gate |
| APR-01 | **PARTIAL / IN PROGRESS**, no accepted product deliverable | Relevant sourced evidence and human review |
| PUX-08 | Future data-to-UX reconciliation step; accepted VDR-01 is available | Follow existing PUX workflow; unsupported semantic/UI elements remain gated |
| VLD-02A / IGR-03 predictive input | **BLOCKED**; `BatchAssessmentInput` NOT ACCEPTED | Accepted VDR-02 plus VLD-02 input decision before implementation |
| Later Viktor target/evaluation work; VLD-02B; analytics engine | **BLOCKED**; target UNKNOWN, baseline/evaluation protocol NOT ACCEPTED | Accepted VDR-02, separately contracted later evidence, and relevant human decisions; domain evidence conditional on the method |
