...
- **Current reconciliation — VLD-R6 post-RBS-01 (2026-09-22):** Reconciles documentation with the integrated RBS-01 implementation (PR #42, merge commit `794eac36b2deb7f244aeccc4ee2326100996edac`). Real single-batch baseline assessment route `GET /api/v1/assessments/{batch_id}`, offline artifact generation, lifespan loading, and health state synchronization are implemented in backend code. Frontend still consumes synthetic demo. This documentation reconciliation makes no new decision and changes no code/schema.
...
| **API Endpoints** | `IMPLEMENTED / ENFORCED CONTRACT` | [`backend/app/api/routes.py`](../backend/app/api/routes.py) | `GET /api/v1/health` returns `HealthResponse` (`status="ok"`, `service="smart-harvest"`, `analytics`). `GET /api/v1/demo/assessment` returns synthetic `RiskAssessment`. `GET /api/v1/assessments/{batch_id}` returns dataset-backed baseline `RiskAssessment` (RBS-01 / PR #42). | Igor (Backend) | Frontend API client (`client.ts`), Smoke tests, Demo runbook | **IMPLEMENTED & ENFORCED IN CODE (RBS-01 / PR #42):** `GET /api/v1/assessments/{batch_id}`, no body; backend owns mapping/provenance. 404 unknown, 409 release-ineligible (Season-2024 training cohort), 503 analytics unavailable/unconfigured, 500 server-owned mapping/internal failure; `Cache-Control: no-store`. Frontend does not yet consume the real route. No POST route accepted. Existing demo and health endpoints remain. | Altering existing route paths, changing HTTP status codes, removing existing endpoints. |
...
| **Production Ingestion & BatchInput** | `ACCEPTED DECISION (ADR 0002) / IMPLEMENTED IN CODE (IGR-03)` | [`backend/app/domain/batch.py`](../backend/app/domain/batch.py), [`backend/app/ingestion/canonical_mapper.py`](../backend/app/ingestion/canonical_mapper.py), [`docs/decisions/0002-predictive-input-semantics.md`](./decisions/0002-predictive-input-semantics.md) | Canonical `BatchAssessmentInput` semantics, 73-field eligibility matrix, and temporal leakage boundary accepted under ADR 0002; typed domain models and deterministic raw→canonical mapper implemented and tested (IGR-03, PR #28). Enforces $T_{assess} \equiv T_{dispatch}$, restricts telemetry to $T_{entry} \le t \le T_{dispatch}$, preserves missingness as `None`, maps planned logistics, and validates fail-closed joins. IGR-04A scoring and IGR-04B internal assessment exist; runtime artifact loader, lifespan context and real route are implemented (RBS-01 / PR #42). | Vladimir (Decision) / Igor (Implementation) | Ingestion pipeline, Analytics engine (future) | Ingestion produces canonical models for analytics consumption. ADR 0005 artifact/snapshot lifecycle and single-batch serving design are implemented; multi-batch endpoints and frontend consumption remain deferred. | Implementing feature engineering, imputation, or analytics scoring inside the ingestion task. |
| **Analytics Baseline & Deterioration Horizon** | `IMPLEMENTED: IGR-04A / IGR-04B / RBS-01 (PR #42)` | `backend/app/analytics/crop_median_baseline.py`, `backend/app/services/baseline_assessment.py`, `backend/app/runtime/`, `backend/artifacts/`, [ADR 0003](./decisions/0003-assessment-evaluation-semantics.md), [ADR 0005](./decisions/0005-runtime-baseline-serving.md) | IGR-04A fits explicit training records and scores canonical inputs; IGR-04B builds RiskAssessment from fitted baseline + input + caller provenance. Null band/horizon/recommendation, empty baseline factors and unavailable reliability persist. Public demo remains synthetic. | Igor / Viktor / Vladimir | Assessment service, future HTTP consumer | ADR 0005 offline artifact generation, lifespan/provider loading and single-batch GET route are implemented (RBS-01 / PR #42). Frontend consumption and multi-batch queue remain deferred. | No new minimum/fallback semantics; valid canonical input the baseline cannot legitimately assess triggers STOP — HUMAN DECISION REQUIRED. |
...
- **Future planned output:** Next bounded slice to be decided by Human Integrator: candidate single-batch frontend integration, or multi-batch/replay backend foundation. Multi-batch/replay, frontend queue, learned/explanation/recommendation/deterioration work and deployment remain separately gated or deferred.
- **Who consumes output:** prscr (API responses for UI consumption), Vladimir (system coherence and test verification).
- **Downstream work blocked until evidence exists:** RBS-01 single-batch serving slice is complete and integrated. Multi-batch/replay queue, frontend real-route consumption, learned models, recommendations, deterioration prediction, and new engine minimum policies remain blocked on their respective decisions/evidence.
...
| **B5: RiskAssessment → API** | FastAPI routes (`backend/app/api/routes.py`) | HTTP response / JSON serialization | `GET /api/v1/health` and `GET /api/v1/demo/assessment` serialized via Pydantic `ContractModel` (`extra="forbid"`). | **OPERATIONAL (FOUNDATION + RBS-01):** `GET /api/v1/health`, `GET /api/v1/demo/assessment`, and real `GET /api/v1/assessments/{batch_id}` (RBS-01 / PR #42). | Realtime source, Persistence needs | Igor (`IGR-01`, API implementation) | Technical verification of serialization, error handling, performance, and endpoint parameterization. | Vladimir (VLD-02 Gate) with Igor | Altering existing public routes; adding unversioned breaking parameters; exposing internal errors. |
...
| Bounded deterministic baseline serving (RBS-01) | **COMPLETE / ACCEPTED / INTEGRATED** through [PR #42](https://github.com/Slave-of-Skynet/training_agrifood/pull/42) | Single-batch GET route, artifact lifecycle, lifespan context, and health synchronization operational in code; frontend consumption and multi-batch queue remain deferred |
...
2. **Deterministic Baseline Serving Implemented; Multi-Batch and Frontend Deferred:** IGR-03 maps canonical inputs, IGR-04A fits/scores, IGR-04B constructs RiskAssessment, and RBS-01 serves single-batch assessments via `GET /api/v1/assessments/{batch_id}` with lifespan artifact/snapshot validation (PR #42). Ranked multi-batch queues, frontend consumption of the real endpoint, learned models, and recommendation engines remain deferred.
3. **Frontend Still Consumes Synthetic Demo:** While the backend provides the real single-batch assessment route, the operator UI currently fetches only `GET /api/v1/demo/assessment` (synthetic fixture) and `GET /api/v1/health`. End-to-end UI consumption of dataset-backed assessments is not yet implemented.
