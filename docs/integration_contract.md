...
This document records the accepted VLD-01 integration contract and workstream dependency map for the SoS team in the Smart Harvest challenge. Its current-state entries include accepted VDR-04A evidence, human-approved ADR 0003 semantics, integrated PUX-08 UX reconciliation (DRAFT FOR REVIEW; governed by ADR 0004 where applicable), integrated IGR-02 raw ingestion and IGR-03 canonical `BatchAssessmentInput` mapping, and human-accepted ADR 0004 MVP product scope (decisions APR2-D1–D6 from APR-02F; source documents 08/09 remain mixed research/spec). IGR-04A fitting/scoring and IGR-04B internal assessment construction are implemented. ADR 0005 accepts the initial serving architecture and future coordinated health-contract change; runtime wiring and the new route remain unimplemented. Public schemas are unchanged by this reconciliation. It connects requirements, domain understanding, dataset reconnaissance, application implementation, evaluation, user experience, demonstration, and final presentation claims. The original pre-recon baseline remains part of project history; the current snapshot and evidence references are recorded in Section 8.
...
| **`Provenance`** | `IMPLEMENTED / ENFORCED CONTRACT` | [`backend/app/domain/assessment.py`](../backend/app/domain/assessment.py), [`frontend/src/api/contracts.ts`](../frontend/src/api/contracts.ts) | Contains `contract_version` (`"1.0.0"`), `engine_tier` (`"fixture"`, `"deterministic_baseline"`, `"learned_model"`), `engine_version`, `generated_at: datetime`, optional `source_dataset_id`, `simulation: bool`, and `notice: str`. | Orchestration / Igor | Frontend UI, Pitch audit, Evaluation logging | **ACCEPTED DECISION / NOT YET WIRED — initial HG-R5 dataset-backed runtime release only:** `engine_tier=deterministic_baseline`, `engine_version=baseline-crop-median-v1-p1-s2024`, `source_dataset_id=training-agrifood-snapshot-v1`, `simulation=true`, notice `SIMULATION / training challenge dataset / deterministic baseline / not production deployment` (ADR 0005 D2/D3). Dataset ID identifies the pinned assessment-source snapshot, not the fit/release; artifact lineage distinguishes training membership. Public schema and fixture remain unchanged; no future global provenance policy is implied. | Omitting simulation flags for synthetic fixtures, misrepresenting fixture results as real model outputs. |
| **API Endpoints** | `IMPLEMENTED / ENFORCED CONTRACT` | [`backend/app/api/routes.py`](../backend/app/api/routes.py) | `GET /api/v1/health` returns `HealthResponse` (`status="ok"`, `service="smart-harvest"`, `analytics="not_configured"`). `GET /api/v1/demo/assessment` returns synthetic `RiskAssessment`. | Igor (Backend) | Frontend API client (`client.ts`), Smoke tests, Demo runbook | **ACCEPTED DECISION / NOT YET IMPLEMENTED — ADR 0005 D5:** `GET /api/v1/assessments/{batch_id}`, no body; backend owns mapping/provenance. 404 unknown, 409 release-ineligible, 503 analytics unavailable, 500 server-owned mapping/internal failure; `Cache-Control: no-store`. No POST route is accepted for this slice. Existing endpoints remain. | Altering existing route paths, changing HTTP status codes, removing existing endpoints. |
| **HealthResponse / analytics readiness** | `IMPLEMENTED: not_configured; ACCEPTED SHARED-CONTRACT CHANGE / NOT YET IMPLEMENTED` | `backend/app/domain/assessment.py`, `frontend/src/api/contracts.ts`, [ADR 0005 D6](./decisions/0005-runtime-baseline-serving.md) | Current backend/TS: `analytics="not_configured"`. Target: `not_configured` (not configured), `ready` (snapshot + artifact loaded/validated), `unavailable` (configured resources failed loading/validation). Top-level `status="ok"` and HTTP 200 represent process/application liveness. | Igor / Vladimir; prscr consumer | Frontend, API tests | Future synchronized backend HealthResponse, TypeScript mirror, API tests and health presentation where applicable; unchanged in this reconciliation. | Further health changes beyond ADR 0005 require review. |
...
| **Production Ingestion & BatchInput** | `ACCEPTED DECISION (ADR 0002) / IMPLEMENTED IN CODE (IGR-03)` | [`backend/app/domain/batch.py`](../backend/app/domain/batch.py), [`backend/app/ingestion/canonical_mapper.py`](../backend/app/ingestion/canonical_mapper.py), [`docs/decisions/0002-predictive-input-semantics.md`](./decisions/0002-predictive-input-semantics.md) | Canonical `BatchAssessmentInput` semantics, 73-field eligibility matrix, and temporal leakage boundary accepted under ADR 0002; typed domain models and deterministic raw→canonical mapper implemented and tested (IGR-03, PR #28). Enforces $T_{assess} \equiv T_{dispatch}$, restricts telemetry to $T_{entry} \le t \le T_{dispatch}$, preserves missingness as `None`, maps planned logistics, and validates fail-closed joins. IGR-04A scoring and IGR-04B internal assessment exist; runtime artifact loader, lifespan context and real route remain absent. | Vladimir (Decision) / Igor (Implementation) | Ingestion pipeline, Analytics engine (future) | Ingestion produces canonical models for analytics consumption. ADR 0005 accepts the initial artifact/snapshot lifecycle and single-batch serving design; wiring remains future work and multi-batch endpoints are deferred. | Implementing feature engineering, imputation, or analytics scoring inside the ingestion task. |
| **Analytics Baseline & Deterioration Horizon** | `IMPLEMENTED: IGR-04A / IGR-04B; ACCEPTED TARGET: ADR 0005 / NOT YET IMPLEMENTED` | `backend/app/analytics/crop_median_baseline.py`, `backend/app/services/baseline_assessment.py`, [ADR 0003](./decisions/0003-assessment-evaluation-semantics.md), [ADR 0005](./decisions/0005-runtime-baseline-serving.md) | IGR-04A fits explicit training records and scores canonical inputs; IGR-04B builds RiskAssessment from fitted baseline + input + caller provenance. Null band/horizon/recommendation, empty baseline factors and unavailable reliability persist. Public demo remains synthetic. | Igor / Viktor / Vladimir | Assessment service, future HTTP consumer | ADR 0005 accepts offline artifact generation and lifespan/provider/single-batch GET; loader and real route do not exist yet. | No new minimum/fallback semantics; valid canonical input the baseline cannot legitimately assess triggers STOP — HUMAN DECISION REQUIRED. |
...
│    ──► next bounded runtime implementation (not implemented)           │
│    ──► later multi-batch/replay ──► later frontend ──► VLD-03          │
│                                                                         │
...
- **Downstream work blocked until evidence exists:** Implementation of final batch selection tables, risk meters, and recommendation widgets is blocked until Igor delivers assessment endpoints under accepted product scope (ADR 0004). PUX-08 is integrated as draft evidence (governed by ADR 0004 where applicable), without bypassing implementation or decision prerequisites.
...
**Current state — VLD-R5-HG1R (2026-09-22):** At `e8e14e3da7921a1d87f7bfc6e474f639ef87aa53`, IGR-04A/04B are implemented; artifact loader, lifespan/provider and real route are not. Vladimir explicitly accepted HG-R5 D1–D7, recorded in ADR 0005. VLD-R5 remains reviewed evidence/decision packet, not wholesale canon. This reconciliation is pending Project Brain review, not a merge claim.
...
**ACCEPTED DECISION — NOT YET IMPLEMENTED:** controlled offline fitting uses `dispatch_datetime < 2025-05-01` (900 Season-2024 batches); only `dispatch_datetime >= 2025-05-01` (900 held-out Season-2025 batches) is initially eligible for assessment/demo. P1 membership is intentionally adopted for this initial artifact, not future production training policy. Requests never fit/refit; startup never silently fits all 1,800 outcomes. Future refits require an explicit release decision and distinct concrete engine version.
...
The next slice is limited to artifact generation/validation, loading, lifespan/provider, single-batch GET, health reconciliation and focused verification. Multi-batch/replay, facility filtering, pagination, frontend queue, database, learned/recommendation/deterioration engines, new confidence/minimum policy, comparability, background retraining, hot reload, deployment and VLD-03 are deferred. If a valid canonical input cannot legitimately be assessed by the existing baseline: **STOP — HUMAN DECISION REQUIRED**. Dataset-backed simulation is not the synthetic fixture; fixture semantics remain unchanged and simulation=false remains outside HG-R5.
...
| IGR-04A / IGR-04B | IMPLEMENTED INTERNALLY | `backend/app/analytics/crop_median_baseline.py`, `backend/app/services/baseline_assessment.py` | Committed reference base `e8e14e3da7921a1d87f7bfc6e474f639ef87aa53`; no HTTP serving claim |
| VLD-R5 / HG-R5 / ADR 0005 | Reviewed RECON evidence; ACCEPTED HUMAN DECISION, serving not implemented | [VLD-R5](./recon/VLD-R5-runtime-assessment-path-recon.md), [ADR 0005](./decisions/0005-runtime-baseline-serving.md) | Explicit Human acceptance 2026-09-22; documentation reconciliation pending review |
...
| Bounded deterministic baseline | **IGR-04A / IGR-04B IMPLEMENTED INTERNALLY** | ADR 0005 initial lifecycle/provenance/API/health accepted; loader, lifespan/provider and route require next bounded implementation contract |
...
2. **Canonical Input, Baseline and Assessment Service Implemented; Serving Wiring Absent:** IGR-03 maps canonical inputs, IGR-04A fits/scores and IGR-04B constructs RiskAssessment internally. Artifact loader, lifespan context/provider and real HTTP route are not implemented. ADR 0005 accepts their initial design; ranked queues, learned models and recommendation engines remain deferred.
3. **Risk Semantics Implemented Internally, Not Yet Served:** IGR-04A/04B implement the accepted crop-median severity score and assessment output. Public demo remains a synthetic `insufficient_data` fixture; ADR 0005 GET and health targets are accepted but not implemented.
...
7. **Artifact Policy Accepted, Not Implemented:** ADR 0005 selects a separate validated JSON baseline artifact and loaded context for the initial release. No loader or persistence layer is configured; no database/ORM/registry is introduced. Broader production persistence remains open.
